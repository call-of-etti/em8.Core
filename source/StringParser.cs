...
namespace CoE.em8.Core
{
    public static class StringParser
    {
        ...
        /// <summary>
        /// Converts a C# literal string into a normal string, i.e. it unescapes the string. "\n" will become a NewLine Character
        /// </summary>
            ...
            while (pos < source.Length)
            {
                ...
                if (c == '\\')
                {
                    ...
                    switch (source[pos])
                    {
                        ...
                        case 'u':
                            pos++;
                            if (pos + 3 >= source.Length) { throw new ArgumentException("Unrecognized escape sequence"); }
                        ...
                        case 'U':                            
                            pos++;
                            if (pos + 7 >= source.Length) { throw new ArgumentException("Unrecognized escape sequence"); }

                            try
                            {
                                uint charValue = UInt32.Parse(source.Substring(pos, 8), NumberStyles.HexNumber);
                                if (charValue > 0xffff) { throw new ArgumentException("Unrecognized escape sequence"); }
        ...
        {
            string result = ParseString(source);

            if (result.Length != 1) { throw new ArgumentException("Invalid char literal"); }
